'use strict';
const userNameInput = document.getElementById('user-name');//ユーザーネーム
...
assessmentButton.addEventListener(//イベント検知を追加
  'click',
  () => {//アロー関数
    const userName = userNameInput.value;//
    if (userName.length === 0) {//名前の入力が空だったら
      return;//関数の処理を終了する
    }
    // 診断結果表示エリアの作成
    ...
    paragraph.innerText = result;//Pタグの内側を設定
    ...
    anchor.setAttribute('href',hrefValue);//属性hfを追加
    ...
    const script = document.createElement('script');//Scriptタグを設定
...
userNameInput.addEventListener(
  ...
  (event) =>{
    if(event.code === 'Enter'){
      assessmentButton.dispatchEvent(new Event('click'));//TODO Enter が押されたときに実行する処理
    }
  }
)
...
for(let i = 0; i < userName.length; i++){//文字数の回数ループ
